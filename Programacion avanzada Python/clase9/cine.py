...
class Cine:
    ...
    def total_entradas_vendidas(self, date):
        ...
        print(
            f'Este el total de entradas vendidas en la fecha {date}: {total_entradas_vendidas}')

    def total_entradas_funcion(self):
        for funcion in self.cartelera:
            print(
                f'Este el total de entradas vendidas para la funcion {funcion.name.title}: {funcion.getTicketVendido()}')
...
while opcion != 0:
    print(f"\n**** Menú ****: ")
    print(f"1. Ver cartelera")
    print(f"2. Comprar entrada")
    print(f"3. Reportes")
    print(f"0. Salir")
    ...
    match opcion:
        case 1:
            ...
            for funcion in cartelera:
                print(f"{funcion.name.title} - {funcion.time} - {funcion.room.number}")
        case 2:
            print("\nSeleccione una función: ")
            cartelera = cineBadPractice.mostrar_cartelera()
            for index,funcion in enumerate(cartelera):
                print(f"{index+1}. {funcion.name.title} - {funcion.time} - {funcion.room.number}")
            ...
            cineBadPractice.venderTicket(fecha, cartelera[indice-1], fila, columna, cartelera[indice-1].room.room_type)
        case 3:
